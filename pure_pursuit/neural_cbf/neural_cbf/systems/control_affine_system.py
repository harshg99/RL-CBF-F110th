...
from neural_cbf.systems.utils import (
    Scenario,
    ScenarioList,
    lqr,
...
class ControlAffineSystem(ABC):
    ...
    def sample_dynamics_data(self, num_samples: int, params: Optional[Scenario] = None) -> torch.Tensor:
        if params is None:
            params_t = copy(self.nominal_params)
        else:
            params_t = copy(params)
        x_max, x_min = self.state_limits
        u_max, u_min = self.control_limits
        ...
        u = torch.Tensor(num_samples, self.n_controls).uniform_(0.0, 1.0)
        for i in range(self.n_controls):
            u[:, i] = u[:, i] * (u_max[i] - u_min[i]) + u_min[i]
        
        xdot = self.closed_loop_dynamics(x, u, params_t) 
